...
//
// EFI configuration table GUID under which the platform FDT is published
// (same GUID as EmbeddedPkg's gFdtTableGuid).
//
STATIC CONST EFI_GUID mFdtTableGuid = {
  ...
  )
{
  ...
  if (GuidHob == NULL) {
    DEBUG ((DEBUG_WARN, "%a: no FDT HOB, firmware DTB not published\n", __func__));
    return;
  }
  ...
  )
{
  ...
  //
  FrontPageTimeout = PcdGet16 (PcdPlatformBootTimeOut);
