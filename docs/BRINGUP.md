...
**当前芯片内容**：出厂 PMON（与 2026-09-15 15:58 的整片备份 `W25Q32_dump_20260915_115813.bin`
MD5 `1bb6d14f1f8c40b084a4d43f76ee6c59` 完全一致），所以这一版还没写进去。

**未写入的原因**：CH341A 现在枚举成 `USB UART-LPT`（VID `0x1A86`，即 UART/LPT 模式，
`1a86:5523`），flashrom 认不出芯片——必须把适配器拨回 SPI 模式（`1a86:5512`）再刷。
