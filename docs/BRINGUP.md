...
## v12（已刷入）——不再猜音高：改成"能不能听见"的判定实验

v11 上机仍然一声都没有。回头把原厂 PMON 的蜂鸣器源码挖出来对照，发现两件事：
...
"没声音"——两者一直无法区分。这一版就是为了把这两种情况**分开**。

### 一、Start.S 里加"三声慢咔嗒"（纯汇编，不用栈、不调函数）

位置在分核之后、设栈之前，直接用未缓存别名 `0x900000001fe00500` 操作 GPIO39：

- 方向寄存器 `+0x04`：`or` 再 `xor` 清 bit7（与 PMON 的 `beep_on` 完全一致）
- 数据寄存器 `+0x14`：高电平保持 `0x4000` 次迭代，低电平 `0x8000` 次，共 3 次

**"慢"是关键**：无论迭代实际多快多慢，压电片都会给出可听的"嗒"。听到 → 复位路径在跑、
GPIO39 通、蜂鸣器好，问题在后面；听不到 → 根本不是音调的问题，别再调频率了。

### 二、`LoongsonBootBeepScale()`：六档音阶（`PreMemInit` 第一件事）
...
| 项 | 值 |
|---|---|
| 真机固件卷 | `~/Downloads/CH341A/uefi_scale1.fd`，983,040 B |
| **MD5** | `9acc53af8a33db12e089ea104117c2b9` |
| QEMU 闭环 | 串口 **59,588 B**（与 v11 字节数完全相同 → 新代码没扰动启动路径）、18 s 到 `Shell>`、GOP 1024x768 ✓ |
| 烧录 | 前 983,040 B 逐字节一致 ✓（2026-09-19 18:53）；刷前回读 MD5 `10d6a278…` 正是 v11 的镜像 → 确认 v11 确实在板上跑过 |

**上机判读**（从上电开始按顺序）：

| 听到 | 结论 |
|---|---|
| 什么也没有 | 复位路径没跑到我们的代码，或 GPIO39/蜂鸣器硬件有问题 → 转串口/LED 路线，别再调音高 |
| 三声慢"嗒" | 复位路径 OK、GPIO39 OK、蜂鸣器 OK |
| 三声"嗒" + 六档音阶 | C 环境正常；**记住第几档最响**，那就是要固定的半周期 |

---

## v13（已刷入）——黑匣子：把进度**写进 flash**，不靠耳朵也不靠显示器

v12 上机仍然全无反应，于是按用户建议**回读芯片**求证。这一读纠正了两个此前的错误结论，
也找到了 flash 日志从来没写进去的**真正原因**。

### 一、回读纠正的两件事

1. **"1MB 以上是 0x00"是假象**。那是 flashrom 带 `-l 布局 -i 区域` 读取时的**填充值**；
   整片无布局读取显示，芯片 1MB 以上真实内容是 **0xFF（出厂擦除态）**。
   所以 `flash_beep3.sh` 里用带布局的回读跟"补齐到 4MB 的镜像"比对毫无意义。
2. **芯片上没有任何运行时写入痕迹**——前 1MB 是我们的固件，1MB 以上与出厂 dump 逐字节一致。
   换句话说："跑了但没留下东西"与"根本没跑"此前**根本无法区分**，这正是卡住的地方。

### 二、flash 日志写不进去的真正原因（对照 PMON `spi_w.c`）

```c
void spi_initw(void) { SPSR=0xc0; PARAM=0x10; PARAM2=0x01; SPER=0x04; SPCR=0x51; }
